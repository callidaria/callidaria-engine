...
public:
	...
	void add(glm::vec2 p,float w,float h,const char* t,unsigned int r,
			unsigned int c,unsigned int f,unsigned int itn)
	{ Anim proc = Anim(p,w,h,t,r,c,f,itn); al.push_back(proc); }
	void load_vertex()
	{
	...
	{
		load_vertex();
		s2d.compile2d("shader/vertex2d.shader",
				"shader/fragment2d.shader");
		load_texture();
	}
	...
	void prepare()
	{
		s2d.enable();reset_shader();glBindVertexArray(vao);
		glDisable(GL_CULL_FACE);glDisable(GL_DEPTH_TEST);glActiveTexture(GL_TEXTURE0);
	}
	...
	void render_anim(int i)
	{
		...
		glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT,
				(void*)((i+sl.size())*6*sizeof(int)));
	}
	void reset_shader()
	{ upload_row(1);upload_col(1);upload_tindex(glm::vec2(0,0)); }
